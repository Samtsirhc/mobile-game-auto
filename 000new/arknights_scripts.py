...
def 登录方舟(emulator: Emulator):
    ...
    while True:
        time.sleep(1)
        emulator.find_and_click(f'下载资源确认')
        emulator.find_and_click(f'start')
        if emulator.find_img(f'鹰角图标'):
            break
    while True:
        time.sleep(1)
        emulator.find_and_click(f'账号管理')
        emulator.find_and_click(f'账号登录')
        if emulator.find_img(f'输入密码'):
            break
    while True:
        if not emulator.find_img(f'确认密码'):
            ...
            if not emulator.find_img(f'已输入密码'):
                ...
                break
    while True:
        emulator.find_and_click(f'登录')
        emulator.find_and_click(f'活动X')
        emulator.find_and_click(f'月卡收货',  (0, 200))
        # emulator.find_and_click(f'签到X')
...
def 画中人(emulator: Emulator):
    ...
    while True:
        if emulator.find_img(f'关卡已选择'):
            break
        emulator.find_and_click(f'画中人')
        emulator.find_and_click(f'入画')
        emulator.find_and_click(f'WR-3')
    pass
...
def 进入基建(emulator: Emulator):
    ...
    emulator.current_dir = dir_name
    while True:
        if emulator.find_img('在基建'):
            break
        emulator.find_and_click(f'基建')
        emulator.find_and_click(f'导航小房子')
        time.sleep(1)
        emulator.find_and_click(f'导航基建')
        time.sleep(1)
    pass
...
def 基建换班(emulator: Emulator):
    ...
    while True:
        ...
        if dorm_order > 3 and switch_state == 0:
            ...
            if switch_step == 2:
                emulator.find_and_click(f'确认')
                emulator.find_and_click(f'红底白勾')
                if emulator.find_img()(f'在进驻总览'):
                    switch_step = 0
        ...
        if emulator.find_img(f'蓝色铃铛'):
            for _ in range(7):
                emulator.find_and_click(
                    f'待办事项', (180, 0))
                time.sleep(0.5)
            emulator.find_and_click(f'蓝色铃铛')
            break
    while True:
...
def 刷经验(emulator: Emulator):
    ...
    while True:
        if emulator.find_img(f'关卡已选择'):
            break
        emulator.find_and_click(f'物资筹备')
        emulator.find_and_click(f'战术演习')
        emulator.find_and_click(f'LS-5')
    logger.info(dir_name)
    pass
...
def 循环挑战(emulator: Emulator):
    ...
    while True:
        if emulator.find_img(f'战斗中'):
            time.sleep(30)
        emulator.find_and_click(f'未代理', (50, 20))
        emulator.find_and_click(f'已代理', (50, 70))
        emulator.find_and_click(f'战斗完成', (0, -200))
        emulator.find_and_click(f'开始行动')
